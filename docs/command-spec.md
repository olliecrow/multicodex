...
- Stores workspace, window, and attachment ownership in a private registry on each host. Persistent random identifiers determine the dedicated tmux server and session names. Reconnection uses those names and verifies instance, window, and workspace ownership markers. Adopted sessions also record their stable tmux session identifier.
...
- Shows every configured project and keeps a workspace visible with no window. Projects with workspaces precede empty projects. Populated projects, their workspaces, and their windows use descending observed terminal activity, with last-use metadata as the fallback before terminal activity exists and stable name order for ties. The sidebar title alternates its live pulse after each completed refresh, including when a host stays offline. Window markers are `●` when the captured terminal changed within five seconds, `·` when the live monitored window is quiet, `×` when its process stopped, and `?` when its host is offline. The selected-window footer also states the status in words. A workspace row uses `!` when its directory is unavailable; its windows keep their independent terminal-state markers. An unavailable directory does not hide or stop a live terminal. It assigns `Option+1` through `Option+9` and `Command+1` through `Command+9` dynamically from current window display order. These shortcuts do not limit the number of windows. Command-key slots are best effort because terminal applications control those keys.
...
- Uses conventional mouse and MacBook keyboard controls instead of a mnemonic command-key mode. Selecting a project or workspace immediately shows its relevant clickable options in the right pane. Selecting a window immediately opens its terminal there. A click also activates visible header buttons, context options, menu items, form fields, and dialog buttons. The Actions menu starts with commands for the selected row and shows only actions whose required project, workspace, window, or terminal exists; a new editor therefore starts with `Add project`. The wheel moves sidebar and menu selection and scrolls tmux terminal history. Clicking the terminal returns input focus to it. `Command+B` or `Ctrl+G` focuses the sidebar. Arrows move one row and open a window when selection reaches it while preserving sidebar focus; `Option+Up` or `Option+Down` moves one screen, and `Command+Up` or `Command+Down` moves to the first or last row with the same behavior. These controls do not require navigation or function keys. `Enter` creates a workspace for a project, creates a window for a workspace, or moves input focus into a selected window. A newly added project is selected with sidebar focus, so `Enter` continues directly to its workspace form. `Command+N` or `Ctrl+N` creates below the selected row, `Command+R` renames a workspace or window, `Tab` opens all Actions, `?` in the sidebar or `Command+?` opens Help, and `Esc` returns to terminal input. `Command+Period` also cancels a dialog. Command-key shortcuts are best effort because the terminal application controls them; visible mouse, Actions, and Control-key alternatives remain available. Renaming changes only the display name and never changes the owned worktree, branch, or tmux session identity. In the sidebar, `Ctrl+C` quits; in terminal input, it is sent to the attached terminal. Actions and destructive confirmations use arrow or Tab selection followed by `Enter`. The Actions menu provides Quit and can send a literal `Ctrl+G` to the attached terminal. Tmux history uses Emacs-style navigation and binds `Option+Up` and `Option+Down` to full-screen paging.
