...
- Shows every configured project and keeps a workspace visible with no window. Each project can have one lazily created managed project terminal in its configured original directory; it never creates a worktree. Projects with terminal windows precede workspace-only projects, which precede empty configured projects. Workspaces with windows precede empty workspaces. Updating entries form a stable name-ordered group at the top of each populated tier; unchanged entries use descending time of their most-recent display change, with last-use metadata only as the fallback before a terminal exists. Each two-second host snapshot hashes the last 100 rendered terminal rows for every managed window, including unselected windows. It excludes tmux selection and activity timestamps, so focus changes and input that leaves the display unchanged cannot renew an updating marker. The marker reports observed display state and never claims to infer Codex turn or foreground-process state: `●` when an applicable live pane differs from its preceding successful snapshot, `○` when its live terminal or terminals are unchanged, `◇` when none exists, `×` when all stopped, and `?` when the host is offline. A new pane is unchanged until a later snapshot differs. A project marker describes only its direct project terminal; the visible workspace and window rows report workspace state independently. The selected-terminal footer states its status in words. The sidebar title keeps stable live or offline health text while refreshes run; only its fixed-width dot pulses. Selecting an offline project or workspace explains automatic reconnection and hides host actions until the host is reachable. A workspace row uses `!` when its directory is unavailable; its windows keep their independent terminal-state markers. An unavailable directory does not hide or stop a live terminal. It assigns `Option+1` through `Option+9` and `Command+1` through `Command+9` dynamically from current terminal display order. These shortcuts do not limit the number of terminals. Command-key slots are best effort because terminal applications control those keys.
