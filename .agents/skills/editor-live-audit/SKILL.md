...
2. Build the exact clean repository commit. Detect each host's operating system and architecture, build once per distinct target, and never copy a client binary to a different target. For cross-host tests, stage the matching binary on every host, replace binaries without stopping active editors, monitors, host processes, or terminal sessions, and verify its file format plus `go version -m` revision and modified state. Restart only an isolated audit editor after all replacements finish.
