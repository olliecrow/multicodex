...
`multicodex editor` is one terminal workspace for local and SSH projects. Clear labeled boundaries separate its project and workspace sidebar, active terminal, and compact Codex weekly-usage box at the bottom of the sidebar. The usage box always shows one labeled row for every distinct configured local Codex account with percent used and compact time until reset, or loading, unavailable, or stale; profiles that resolve to the same account identity share one row. It never collapses accounts behind a count. Long labels keep both ends visible. If all rows cannot fit while keeping the workspace list usable, the editor states the required height instead of hiding data. Account refreshes never reduce or resize the active terminal. Projects with terminal windows stay above workspace-only projects, which stay above empty configured projects. Workspaces with windows stay above empty workspaces. Updating entries form a stable name-ordered group at the top of each populated tier; unchanged entries use their most-recent display change. This prevents updating projects from exchanging positions on each two-second sample. Each snapshot hashes the last 100 rendered terminal rows and ignores tmux selection timestamps, so selecting or interacting with a finished pane does not renew its signal. The signal reports observed display state, not a guessed Codex conversation or process state: `●` means the rendered pane changed since the preceding successful snapshot, `○` means its live terminal or terminals were unchanged, `◇` means it has no terminal, `×` means all its terminals stopped, and `?` means the host is offline. A project row describes only its direct project terminal; its visible workspace and window rows show workspace state independently. A selected window also states its status in words in the footer. A workspace row shows `!` when its directory is unavailable while its window signals continue to show terminal state. The sidebar title keeps stable live or offline health text during refresh; only its fixed-width dot pulses.
